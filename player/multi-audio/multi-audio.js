...
class MultiAudioTrackManager {
    ...
    /**
     * Detect external MKA sidecar tracks in .tracks folder.
     * Supported names:
     * - videoName.mka
     * - videoName_trackN.mka
     */
    async detectExternalMkaTracks() {
        const externalTracks = [];

        if (!this.tracksDirectoryHandle || typeof this.tracksDirectoryHandle.entries !== 'function') {
            return externalTracks;
        }

        const baseName = this.videoFileName.replace(/\.[^/.]+$/, '');
        const escapedBase = baseName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const plainNameRe = new RegExp(`^${escapedBase}\\.mka$`, 'i');
        const indexedNameRe = new RegExp(`^${escapedBase}_track(\\d+)\\.mka$`, 'i');

        const candidates = [];
        for await (const [entryName, handle] of this.tracksDirectoryHandle.entries()) {
            if (handle.kind !== 'file') continue;

            if (plainNameRe.test(entryName)) {
                candidates.push({
                    fileName: entryName,
                    sortOrder: 0,
                    displayOrder: 1
            ...
            const indexedMatch = entryName.match(indexedNameRe);
            if (indexedMatch) {
                const parsed = Number.parseInt(indexedMatch[1], 10);
        ...
        candidates.sort((a, b) => a.sortOrder - b.sortOrder || a.fileName.localeCompare(b.fileName));
        ...
        candidates.forEach((candidate, idx) => {
            externalTracks.push({
                ...
                streamIndex: -1,
                type: 'audio',
                label: `MKA Track ${candidate.displayOrder}`,
                language: 'external',
                codec: 'mka',
                external: true,
                externalFileName: candidate.fileName
            });
        });

        if (externalTracks.length > 0) {
            maDebug.log(`[MultiAudio] Found ${externalTracks.length} external MKA track(s) in .tracks`);
        }
    ...
    async checkDiskCache(trackIndex) {
        ...
        try {
            ...
            const trackFileNames = [
                `${baseName}_track${trackIndex}.mp4`,
                `${baseName}_track${trackIndex}.mka`
            ];
     ...
     */
    async saveToDisk(trackIndex, audioBlob, fileExtension = 'mp4') {
        ...
        try {
            const baseName = this.videoFileName.replace(/\.[^/.]+$/, '');
            const safeExtension = String(fileExtension || 'mp4').replace(/^\./, '').toLowerCase();
            const trackFileName = `${baseName}_track${trackIndex}.${safeExtension}`;
    ...
    async analyzeAudioTracks(file) {
        ...
        try {
            ...
            const externalTracks = await this.detectExternalMkaTracks();
            if (externalTracks.length > 0) {
                const startIndex = this.audioTracks.length;
                externalTracks.forEach((track, offset) => {
                    track.index = startIndex + offset;
                });
                this.audioTracks.push(...externalTracks);
            }
...
async extractAllAudioTracks(inputFileName) {
    ...
    for (let i = 0; i < this.audioTracks.length; i++) {
        ...
        const audioMimeType = outputFormat === 'matroska' ? 'audio/x-matroska' : 'audio/mp4';
        const audioBlob = new Blob([audioData.buffer], { type: audioMimeType });
    ...
    async extractSingleAudioTrack(inputFileName, trackIndex) {
        ...
        const selectedTrack = this.audioTracks[trackIndex];
        if (selectedTrack?.external && selectedTrack.externalFileName) {
            maDebug.log(`[MultiAudio] Loading external MKA track: ${selectedTrack.externalFileName}`);
            const externalFile = await this.loadExternalTrackFile(selectedTrack.externalFileName);
            if (!externalFile) {
                throw new Error(`External track not found: ${selectedTrack.externalFileName}`);
            }
            ...
            const audioURL = this.createTrackedBlobURL(externalFile);
            const audioElement = new Audio(audioURL);
            audioElement.volume = this.video.volume;
            audioElement.muted = true;
            audioElement.preload = 'auto';
        ...
        await this.saveToDisk(trackIndex, audioBlob, outputFormat === 'matroska' ? 'mka' : 'mp4');
    ...
    /**
     * Load external sidecar audio file from .tracks directory
     */
    async loadExternalTrackFile(fileName) {
        if (!this.tracksDirectoryHandle) return null;

        try {
            let fileHandle;
            if (typeof this.tracksDirectoryHandle.getFileHandle === 'function') {
                fileHandle = await this.tracksDirectoryHandle.getFileHandle(fileName);
            } else if (typeof this.tracksDirectoryHandle.getFile === 'function') {
                fileHandle = await this.tracksDirectoryHandle.getFile(fileName);
            } else {
                return null;
            }

            return await (fileHandle.getFile ? fileHandle.getFile() : fileHandle);
        } catch (error) {
            console.warn('[MultiAudio] Failed to load external track:', fileName, error);
            return null;
        }
    }
