# Youvi Player (by Yuvisite)
[![22Novyj-proekt(1)(3)(1)(1)(1)-(4).png](https://i.postimg.cc/N0dthTVS/22Novyj-proekt(1)(3)(1)(1)(1)-(4).png)](https://postimg.cc/R3JyK3vL)


Youvi Player is an offline video library with video hosting interface, featuring danmaku, comments, tags (including aliases and implications), subtitles and more. Fully local and portable (localhost needed for subtitles and multiple audio tracks), works without internet. Download videos with danmaku from Youtube, Niconico and Bilibili and watch in videohosting UI, or add you own videos, or all together!
...
There are scripts (and now also built-in download tab) for importing comments/danmaku into Youvi Player JSON files. You can import YouTube comments as regular comments and danmaku (from timestamped comments or live chat), import Niconico timed comments/danmaku track (the same stream can be saved as Youvi Player comments list or as Youvi Player danmaku), and import native danmaku from Bilibili. This is one of the reasons why this thing was made. And you can also obviously write comments and danmaku by yourself. 
...
**Downloads** - Download video from other sites to Youvi Player
...
[![image.png](https://i.postimg.cc/131KGkr1/image.png)](https://postimg.cc/Yh8gkyPd)
...
## Themes
Site have themes and custom background for channels and videopages. To get this add channel tag under video, go to chanel page and in header for background click BG button, for themes dropdown with text Default, choose from list.
[![image.png](https://i.postimg.cc/fLrsbDTC/image.png)](https://postimg.cc/MMVNPkBf)
[![image.png](https://i.postimg.cc/qMQPBpbv/image.png)](https://postimg.cc/ykgQvCCM)

## Youvi Player Main

This is the main page. Huge banner with logo. Under the banner are categories. Categories are a tag of type ct, I selected certain categories for classification, videos from which will appear here. Buttons 2-7 - number of cards per row.
...
Multiple audio tracks are extracted from MKV files: click the wave button in the player and select the needed one, wait and audio will play. Extracted cache is saved in `.tracks` folder next to video.

External MKA sidecar tracks are also supported from `.tracks` folder. Use one of these names:
- `video_name.mka`
- `video_name_track0.mka`, `video_name_track1.mka`, etc.
...
Niconico: native timed comments/danmaku track, description. `Comments` saves it as Youvi Player comments, `Danmaku` saves the same track as Youvi Player danmaku.  
